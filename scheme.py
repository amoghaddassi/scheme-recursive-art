...
def eval_all(expressions, env, tail=False):
    ...
    if expressions == nil:
        return None
    first, rest = expressions.first, expressions.second
    if rest == nil:
        return scheme_eval(first, env, tail)
    else:
        scheme_eval(first, env)
        return eval_all(rest, env, tail)
    # END PROBLEM 8
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('Length of formals does not match length of values.')
        else:
            child = Frame(self)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Incorrect arguments for procedure.')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        res = do_lambda_form(Pair(target.second, expressions.second), env)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        param, arg = bindings.first.first, scheme_eval(bindings.first.second.first, env)
        bindings = bindings.second
        formals, args = Pair(param, formals), Pair(arg, args)
    check_formals(formals)
