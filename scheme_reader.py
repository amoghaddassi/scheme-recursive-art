...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        operator = quotes[val]
        res = scheme_read(src)
        return Pair(operator, Pair(res, nil))
        # END PROBLEM 7
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 2
            src.remove_front()
            first, second = scheme_read(src), src.remove_front()
            if second != ')':
                raise SyntaxError('Dotted expressions must only have exactly one item after the dot.')
            else:
                return first
            # END PROBLEM 2
        else:
